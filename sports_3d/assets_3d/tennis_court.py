...
Coordinate system:
  - X: along the court (positive = one baseline, negative = other baseline)
  - Y: up (height)
  - Z: across the court (sideline to sideline)
...
class TennisCourt3D:
    ...
    def _add_rectangle(self, x_range: Tuple[float, float], 
                       ...
                       z: float = 0, color: str = "white"):
        """Add a rectangle (4 lines) to the court."""
        x1, x2 = x_range
        y1, y2 = y_range
        self._add_line((x1, y1, z), (x2, y1, z), color)
        self._add_line((x2, y1, z), (x2, y2, z), color)
        self._add_line((x2, y2, z), (x1, y2, z), color)
        self._add_line((x1, y2, z), (x1, y1, z), color)
    ...
    def _build_court(self):
        ...
        # Baseline 1 (positive X side)
        self._add_line(
            (d.half_length, 0, -width),
            (d.half_length, 0, width)
        )
        
        # Baseline 2 (negative X side)
        self._add_line(
            (-d.half_length, 0, -width),
            (-d.half_length, 0, width)
        )
        ...
        # Singles sidelines
        self._add_line(
            (-d.half_length, 0, -d.half_singles_width),
            (d.half_length, 0, -d.half_singles_width)
        )
        self._add_line(
            (-d.half_length, 0, d.half_singles_width),
        ...
        if self.include_doubles:
            self._add_line(
                (-d.half_length, 0, -d.half_doubles_width),
                (d.half_length, 0, -d.half_doubles_width)
            )
            self._add_line(
                (-d.half_length, 0, d.half_doubles_width),
        ...
        # Service lines (parallel to net, 6.4m from it on each side)
        self._add_line(
            (d.service_line_distance, 0, -d.half_singles_width),
            (d.service_line_distance, 0, d.half_singles_width)
        )
        self._add_line(
            (-d.service_line_distance, 0, -d.half_singles_width),
        ...
        # Center marks on baselines (small perpendicular lines)
        center_mark_length = 0.1  # 10cm
        self._add_line(
            (d.half_length, 0, -center_mark_length),
            (d.half_length, 0, center_mark_length)
        )
        self._add_line(
            (-d.half_length, 0, -center_mark_length),
        ...
        # Net line (at x=0, for reference - actual net would be a mesh)
        self._add_line(
            (0, 0, -d.half_doubles_width),
            (0, 0, d.half_doubles_width),
            color="gray"
        )
    ...
    def get_surround_vertices(self, runoff_back: float = 6.4, runoff_side: float = 3.66):
        ...
        inner_half_x = d.half_length
        inner_half_z = d.half_doubles_width if self.include_doubles else d.half_singles_width
        
        # Outer boundary (with runoff)
        outer_half_x = inner_half_x + runoff_back
        outer_half_z = inner_half_z + runoff_side
    ...
    def to_plotly_figure(self, 
        ...
        if show_surface:
            ...
            y_surf = -0.01  # Slightly below lines (Y is now height)
            
            d = self.dims
            inner_hx = d.half_length
            inner_hz = d.half_doubles_width if self.include_doubles else d.half_singles_width
            outer_hx = inner_hx + runoff_back
            outer_hz = inner_hz + runoff_side
            
            # Back strip (behind positive X baseline)
            back1 = np.array([
                [inner_hx, y_surf, -outer_hz],
            ...
            # Back strip (behind negative X baseline)
            back2 = np.array([
                [-outer_hx, y_surf, -outer_hz],
                [-inner_hx, y_surf, -outer_hz],
                [-inner_hx, y_surf, outer_hz],
            ...
            court_surface[:, 1] = y_surf  # Y is now height
            fig.add_trace(go.Mesh3d(
                x=court_surface[:, 0],
                y=court_surface[:, 1],
                z=court_surface[:, 2],
        ...
        # Configure layout
        d = self.dims
        outer_hx = d.half_length + runoff_back
        outer_hz = d.half_doubles_width + runoff_side
        
        fig.update_layout(
            scene=dict(
                xaxis=dict(
                    title="X (along court) [m]",
...
def create_sample_ball_trajectory() -> np.ndarray:
    ...
    # X: along court, Y: height, Z: across court
    x0, y0, z0 = 11.0, 2.5, 2.0  # Serve height ~2.5m
    
    # Initial velocity (toward other baseline)
    vx0, vy0, vz0 = -25.0, 5.0, -3.0  # m/s
    
    # Gravity (acts on Y axis now)
    g = 9.81
